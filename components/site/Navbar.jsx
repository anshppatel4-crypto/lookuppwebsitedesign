        ...
        <nav className={cn('flex h-[70px] w-full max-w-[1150px] items-center justify-between rounded-full border border-[#d6def8] bg-white/80 px-7 shadow-[0_18px_45px_-20px_rgba(48,85,207,0.42)] backdrop-blur-xl transition-all sm:px-8', scrolled && 'h-[62px] bg-white/90')}>
          ...
          <div className="hidden items-center gap-2 md:flex">
            {NAV_LINKS.map((link) => {
              ...
              return <Link key={link.href} href={link.href} className={cn('rounded-full px-5 py-3 text-[21px] font-medium transition', active ? 'bg-[#edf1fb] text-[#3055cf]' : 'text-[#252548]/75 hover:text-[#252548]')}>{link.name}</Link>
            })}
          </div>
          <div className="flex items-center gap-2">
            <Link href={SITE.donate} target="_blank" rel="noreferrer" className="hidden items-center gap-3 rounded-full bg-gradient-to-r from-[#116dff] to-[#3055cf] px-8 py-4 text-xl font-semibold text-white shadow-[0_12px_24px_-12px_rgba(48,85,207,0.75)] transition hover:-translate-y-0.5 md:flex">
